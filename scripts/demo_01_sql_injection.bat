...
chcp 65001 >nul
REM =============================================================================
REM  scripts/demo_01_sql_injection.bat - Demo kich ban 1: SQL Injection
REM =============================================================================
REM  Kich ban nay minh hoa:
REM    - Loi SQL Injection (CWE-89)
...
REM    cd github_demo
REM    scripts\demo_01_sql_injection.bat
REM =============================================================================
...
REM ===== CAU HINH =====
set SO_KICH_BAN=1
set TEN_KICH_BAN=SQL Injection (CWE-89)
set CWE=CWE-89
set XEP_HANG=Tier 1 - CWE Top 25
set CVE_LIEN_QUAN=CVE-2019-9194 (PostgreSQL)
...
set FILE_VULN=%REPO_ROOT%\scenarios\01\auth_vuln.c
set FILE_SAFE=%REPO_ROOT%\scenarios\01\auth_safe.c
...
echo.
echo [1.1] Copy file co loi: scenarios\01\auth_vuln.c -^> auth.c
...
echo.
echo [1.4] git commit -m "add sql_injection vulnerability"
echo       (Hook se tu dong chay SCGL de phan tich...)
git commit -m "add sql_injection vulnerability" 2>&1
set RC=%errorlevel%
echo.
if %RC% neq 0 (
    echo +================================================================+
...
echo.
echo [2.1] Copy file an toan: scenarios\01\auth_safe.c -^> auth.c
...
git add "%REPO_ROOT%\auth.c" 2>nul
echo.
echo [2.3] git commit -m "fix sql_injection - use safe pattern"
echo       (Hook se tu dong chay SCGL de phan tich...)
git commit -m "fix sql_injection - use safe pattern" 2>&1
set RC=%errorlevel%
echo.
if %RC% equ 0 (
    echo +================================================================+
