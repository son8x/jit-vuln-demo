...
chcp 65001 >nul
REM =============================================================================
REM  scripts/demo_05_hardcoded_credentials.bat - Demo kich ban 5: Hardcoded Credentials
REM =============================================================================
REM  Kich ban nay minh hoa:
REM    - Loi Hardcoded Credentials (CWE-798)
...
REM    cd github_demo
REM    scripts\demo_05_hardcoded_credentials.bat
REM =============================================================================
...
REM ===== CAU HINH =====
set SO_KICH_BAN=5
set TEN_KICH_BAN=Hardcoded Credentials (CWE-798)
set CWE=CWE-798
set XEP_HANG=Tier 3 (cau hinh)
set CVE_LIEN_QUAN=lo secret tren GitHub
...
set FILE_VULN=%REPO_ROOT%\scenarios\05\cred_vuln.c
set FILE_SAFE=%REPO_ROOT%\scenarios\05\cred_safe.c
...
echo.
echo [1.1] Copy file co loi: scenarios\05\cred_vuln.c -^> auth.c
...
echo.
echo [1.4] git commit -m "add hardcoded_credentials vulnerability"
echo       (Hook se tu dong chay SCGL de phan tich...)
git commit -m "add hardcoded_credentials vulnerability" 2>&1
set RC=%errorlevel%
echo.
if %RC% neq 0 (
    echo +================================================================+
...
echo.
echo [2.1] Copy file an toan: scenarios\05\cred_safe.c -^> auth.c
...
git add "%REPO_ROOT%\auth.c" 2>nul
echo.
echo [2.3] git commit -m "fix hardcoded_credentials - use safe pattern"
echo       (Hook se tu dong chay SCGL de phan tich...)
git commit -m "fix hardcoded_credentials - use safe pattern" 2>&1
set RC=%errorlevel%
echo.
if %RC% equ 0 (
    echo +================================================================+
