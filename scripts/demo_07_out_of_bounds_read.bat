...
chcp 65001 >nul
REM =============================================================================
REM  scripts/demo_07_out_of_bounds_read.bat - Demo kich ban 7: Out-of-bounds Read
REM =============================================================================
REM  Kich ban nay minh hoa:
REM    - Loi Out-of-bounds Read (CWE-125)
...
REM    cd github_demo
REM    scripts\demo_07_out_of_bounds_read.bat
REM =============================================================================
...
REM ===== CAU HINH =====
set SO_KICH_BAN=7
set TEN_KICH_BAN=Out-of-bounds Read (CWE-125)
set CWE=CWE-125
set XEP_HANG=#3 CWE Top 25 - Heartbleed
set CVE_LIEN_QUAN=CVE-2014-0160 (Heartbleed)
...
set FILE_VULN=%REPO_ROOT%\scenarios\07\oob_read_vuln.c
set FILE_SAFE=%REPO_ROOT%\scenarios\07\oob_read_safe.c
...
echo.
echo [1.1] Copy file co loi: scenarios\07\oob_read_vuln.c -^> auth.c
...
echo.
echo [1.4] git commit -m "add out_of_bounds_read vulnerability"
echo       (Hook se tu dong chay SCGL de phan tich...)
git commit -m "add out_of_bounds_read vulnerability" 2>&1
set RC=%errorlevel%
echo.
if %RC% neq 0 (
    echo +================================================================+
...
echo.
echo [2.1] Copy file an toan: scenarios\07\oob_read_safe.c -^> auth.c
...
git add "%REPO_ROOT%\auth.c" 2>nul
echo.
echo [2.3] git commit -m "fix out_of_bounds_read - use safe pattern"
echo       (Hook se tu dong chay SCGL de phan tich...)
git commit -m "fix out_of_bounds_read - use safe pattern" 2>&1
set RC=%errorlevel%
echo.
if %RC% equ 0 (
    echo +================================================================+
