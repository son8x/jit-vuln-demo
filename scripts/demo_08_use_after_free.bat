...
chcp 65001 >nul
REM =============================================================================
REM  scripts/demo_08_use_after_free.bat - Demo kich ban 8: Use After Free
REM =============================================================================
REM  Kich ban nay minh hoa:
REM    - Loi Use After Free (CWE-416)
...
REM    cd github_demo
REM    scripts\demo_08_use_after_free.bat
REM =============================================================================
...
REM ===== CAU HINH =====
set SO_KICH_BAN=8
set TEN_KICH_BAN=Use After Free (CWE-416)
set CWE=CWE-416
set XEP_HANG=#5 CWE Top 25 - BlueKeep
set CVE_LIEN_QUAN=CVE-2019-0708 (BlueKeep)
...
set FILE_VULN=%REPO_ROOT%\scenarios\08\uaf_vuln.c
set FILE_SAFE=%REPO_ROOT%\scenarios\08\uaf_safe.c
...
echo.
echo [1.1] Copy file co loi: scenarios\08\uaf_vuln.c -^> auth.c
...
echo.
echo [1.4] git commit -m "add use_after_free vulnerability"
echo       (Hook se tu dong chay SCGL de phan tich...)
git commit -m "add use_after_free vulnerability" 2>&1
set RC=%errorlevel%
echo.
if %RC% neq 0 (
    echo +================================================================+
...
echo.
echo [2.1] Copy file an toan: scenarios\08\uaf_safe.c -^> auth.c
...
git add "%REPO_ROOT%\auth.c" 2>nul
echo.
echo [2.3] git commit -m "fix use_after_free - use safe pattern"
echo       (Hook se tu dong chay SCGL de phan tich...)
git commit -m "fix use_after_free - use safe pattern" 2>&1
set RC=%errorlevel%
echo.
if %RC% equ 0 (
    echo +================================================================+
